...
impl<P: Position, V: Vector> Block<P> for Block6<V> {
    ...
    fn get_chridx_of(&self, rem: u32) -> u8 {
        ...
        let v6 = (self.0[4] >> V::from_u32(mov)).as_u8() & 1;
        v1 + (v2 << 1) + (v3 << 2) + (v4 << 3) + (v5 << 4) + (v6 << 5)
    }
}

unsafe impl<V: Vector> Zeroable for Block6<V> {}
unsafe impl<V: Vector + 'static> Pod for Block6<V> {}