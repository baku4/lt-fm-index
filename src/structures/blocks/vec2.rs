        ...
        impl $name {
            ...
            fn vectorize(
                ...
                text_chunk.iter().for_each(|c| {
                    let chridx = chr_idx_table.idx_of(*c);
                    rank_checkpoint[chridx as usize] += 1;
                    bwt_vector[0] <<= 1;
                    bwt_vector[0] += (chridx & 0b01) as $bits;
                    bwt_vector[1] <<= 1;
                    bwt_vector[1] += (chridx & 0b10) as $bits;
                });
            }
            #[inline]
            ...
            fn get_rank_and_chridx_of_rem_inner(&self, rem: u64) -> (u64, u8) {
                ...
                let v1 = (self.bwt_vector[0] >> mov as $bits) as u8;
                let v2 = (self.bwt_vector[1] >> mov as $bits) as u8;
                let chridx = v1 + v2;
